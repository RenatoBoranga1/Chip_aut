...
def build_coverage(collection_id, database, destination, rules_path=DEFAULT_MATCHING_RULES):
    ...
    for ad, run in zip(ads, runs):
        result = run["result"]
        if result.match_type in {"NAO_ENCONTRADA_NA_BASE", "CORRESPONDENCIA_PROVAVEL"}:
        ...
        if group not in groups:
            group = "REVISAR"
        groups[group].append(
            {"advertisement": asdict(ad), "matching_run_id": run["run_id"], "matching": asdict(result)}
        )
    report = {
    ...
    for group, items in groups.items():
        ...
        for item in items:
            ad, result = item["advertisement"], item["matching"]
            candidates = "; ".join(
